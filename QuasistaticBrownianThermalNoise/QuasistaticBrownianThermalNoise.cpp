...
class ElasticProblem {
 ...
 private:
  ...
  // Young tensors Yijkl
  // We also define youngs tensors for the same matarials
  // that would allow the loss angles to be varied.
  // The tensors with the phi appended to the end of the name are defined in
  // terms of the bulk and shear modulus, to allow for the loss angles to be
  // varied while the other tensors are only defined in terms of the
  ...
  // Loss Angles for the Substrates
  double lossPhi_AlGaAs, lossPhi_Iso_AlGaAs, lossPhi_Iso_Ta2O5,
      lossPhi_Iso_FusedSilica;
...
NeumannValues<dim>::NeumannValues(const double &inR0, const double &inF0)
    : Function<dim>(dim), r0(inR0), F0(inF0) {}
...

{
  ...
  // kIso_Ta2O5 #tantalum
  // kIso_FusedSilica #fused silica
  mWhichCoatingYijkl = kIso_AlGaAs;
  ...
  // These are the loss angles that we use to vary Crystalline AlGaAs with
  // multiple loss angles. We set these equal to

  phi11 = 2.5e-5;
  phi12 = 2.5e-5;
  phi44 = 2.5e-5;

  // The variables declared here are used vary the bulk and shear modulus
  // for the three materials allowed in the code. There is no experimentally
  // measured values for these two paremeters, so we set them equal to each
  // other.

  // The value that we take is from Cole+ (2013)

  lossPhi_Iso_FusedSilica_bulk = 1.e-6;
  lossPhi_Iso_FusedSilica_shear = 1.e-6;

  // The values we use are from Hong + et. al.
  lossPhi_Ta2O5_bulk = 2.3e-4;
  lossPhi_Ta2O5_shear = 2.3e-4;

  // Values taken from Cole+ (2013)

  lossPhi_Iso_AlGaAs_bulk = 2.5e-5;
  lossPhi_Iso_AlGaAs_shear = 2.5e-5;
  ...
  // K = lambda + 2/3 mu
  // where K is the bulk modulus, lambda is the first lame coefficient, and mu
  // is the shear modulus/ second lame coeffiecient.
